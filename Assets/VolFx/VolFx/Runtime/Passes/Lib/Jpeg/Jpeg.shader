    ...
    SubShader
    {
        ...
        Pass // 0
        {
            ...
            float4 frag(v2f i) : SV_Target
            {
                ...
                // Distortion
                float4 d = tex2D(_DistortionTex, uv * _DistortionScale + _DistortionOffset) - float4(.5, .5, 0, 0);
                ...
                // === Sample MainTex ===
                float4 rgbaY = tex2D(_MainTex, uvY);        // RGB for luma
                float4 rgbaC = tex2D(_MainTex, blockUV);    // RGB for chroma
                ...
                float4 glitchCol;
                {
                    float2 uvG = uv + dir * strength * _ApplyDistortGlitch;
